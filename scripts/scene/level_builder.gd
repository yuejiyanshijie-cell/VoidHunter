...
func _create_ground() -> void:
	...
	ground.name = "Ground"
	ground.position = Vector2(GROUND_WIDTH / 2, GROUND_Y + GROUND_HEIGHT / 2)
	ground.collision_layer = 1
	ground.collision_mask = 0

	var coll: CollisionShape2D = CollisionShape2D.new()
	var rect: RectangleShape2D = RectangleShape2D.new()
	rect.size = Vector2(GROUND_WIDTH, GROUND_HEIGHT)
	coll.shape = rect
	ground.add_child(coll)

	var vis: ColorRect = ColorRect.new()
	vis.color = FLOOR_COLOR
	vis.size = Vector2(GROUND_WIDTH, GROUND_HEIGHT)
	vis.position = Vector2(-GROUND_WIDTH / 2, -GROUND_HEIGHT / 2)
	ground.add_child(vis)

	# 地面发光顶部线
	var glow: ColorRect = ColorRect.new()
	glow.color = Color(0.2, 0.15, 0.5, 0.4)
	glow.size = Vector2(GROUND_WIDTH, 2)
	glow.position = Vector2(-GROUND_WIDTH / 2, -GROUND_HEIGHT / 2)
